...
class MaskedMLP(nn.Module):
    ...
    def forward(self, input):
        ...
        input_t = input.t()
        # pruned_input = input[:, self.boolmask]#由于weight剪枝，input对应列需要进行移除
        pruned_input = input_t[self.boolmask,:].t() #行移除较快
...
#         self.weight=nn.Parameter(masked_weight)#将weight变为[-1，cout]形状
#         self.idx = nn.Parameter(idx,requires_grad=False)
...
#         #masked_weight = torch.einsum('ij,j->ij', self.weight, mask)
#         #output = torch.nn.functional.linear(input, masked_weight, self.bias)
#         pruned_input = torch.index_select(input,-1,self.idx)#由于weight剪枝，input对应列需要进行移除,在vit模型中，input有可能是[bs, seqlen, inputsize]形状因此此处使用-1
#         # output = torch.mm(pruned_input,self.weight)
#         output = pruned_input @ self.weight
#         return output