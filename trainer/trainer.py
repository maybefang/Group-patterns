...
class Trainer():
    ...
    def sparse_train(self, model, loss, device, tr_loader, va_loader=None, optimizer=None, scheduler=None):
        args = self.args
        logger = self.logger
        ...
        for epoch in range(1, args.max_epoch+1):
            ...
            filename = os.path.join(args.model_folder, 'model-'+str(epoch)+'.pth')
            maskname = os.path.join(args.model_folder, 'mask-'+str(epoch)+'.pkl')
            save_model(model, filename, maskname)
            if scheduler is not None:
                scheduler.step()
        logger.info(">>>>> Training process finish")
